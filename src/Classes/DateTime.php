...
class DateTime
{
    ...
    public static function convertDataToUtc($data, array $arrayKey = [], $format = 'Y-m-d H:i:s')
    {
        try {
            ...
            $fromTimeZone = \ProcessMaker\Package\PackagePinnacle\Models\User::getUserLoggedTimeZone();
            $toTimeZone = 'UTC';

            //Return
            return (new \ProcessMaker\Package\PackagePinnacle\Classes\DateTime())->convertDataToTimeZone($data, $fromTimeZone, $toTimeZone, $arrayKey, $format);
        } catch (\Exception $e) {
            throw $e;
        }
    }
    ...
    public static function convertUtcToTimeZone($data, $toTimeZone = null, array $arrayKey = [], $format = 'Y-m-d H:i:s')
    {
        try {
            $fromTimeZone = 'UTC';
            $toTimeZone = (!is_null($toTimeZone)) ? $toTimeZone : \ProcessMaker\Package\PackagePinnacle\Models\User::getUserLoggedTimeZone();
    ...
    public static function convertUtcToIso8601($data, array $arrayKey = [])
    {
        try {
            ...
            $fromTimeZone = 'UTC';
            $toTimeZone = \ProcessMaker\Package\PackagePinnacle\Models\User::getUserLoggedTimeZone();

            //Return
            return (new \ProcessMaker\Package\PackagePinnacle\Classes\DateTime())->convertDataToIso8601($data, $fromTimeZone, $toTimeZone, $arrayKey);
        } catch (\Exception $e) {
            throw $e;
        }
    }
